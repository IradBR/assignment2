...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Class, Callback> message_call;
    ...
    public MicroService(String name) {
        this.name = name;
        message_call= new ConcurrentHashMap();
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        message_call.putIfAbsent(type,callback);
    }
    ...
    public final void run() {
        ...
        while (!terminated) {
            try{
                message= message_Bus.awaitMessage(this);
            }catch (InterruptedException e){e.printStackTrace();}
            if(message!=null){
                Callback c= message_call.get(message.getClass());
                if(c!=null){
                    c.call(message);
                }
            }
